...
ARCHITECTURE
------------
  [Web Browser / VXML Gateway]
          |
          v
  [FastAPI Integration Layer]  ← THIS FILE
          |
          |── GET  /              → Serves IVR Web Simulator UI
          |── POST /ivr/start     → Welcome prompt + session creation
          |── POST /ivr/input     → Universal DTMF input handler
          ...
          |── POST /acs/bridge    → ACS/BAP connector stub (M3 replaces)
          |── GET  /health        → Health check for M4 deployment
          v
  [ACS / BAP — Conversational AI]  ← Milestone 3 plugs in here

FUTURE MILESTONE HOOKS
----------------------
- M3: Replace /acs/bridge stub with real Azure ACS SDK
- M3: Add NLU intent mapping to each menu item's "acs_event"
- M4: Replace SESSIONS dict with Redis for production scaling
- M4: Add real IRCTC API calls in PNR/Booking/Tracking endpoints
=============================================================================
"""
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="IRCTC Smart IVR — Integration Layer",
...
# ---------------------------------------------------------------------------
# SESSION STORE
# TODO (M4): Replace with Redis → import redis; r = redis.Redis(...)
# ---------------------------------------------------------------------------
SESSIONS: dict = {}

# ---------------------------------------------------------------------------
# IRCTC MENU CONFIG — add/remove services here without touching endpoints
# M3 HOOK: "acs_event" maps directly to ACS bot activity triggers
# ---------------------------------------------------------------------------
MAIN_MENU = {
...
class StartRequest(BaseModel):
    ...
    acs_call_connection_id: Optional[str] = None   # M3: ACS passes this

class DTMFRequest(BaseModel):
    session_id: str
    digit: str
    current_flow: str
    acs_recognize_result: Optional[dict] = None    # M3: ACS RecognizeCompleted
...
class ACSBridgeRequest(BaseModel):
    """
    VXML → ACS/BAP bridge.
    M3 TODO: Replace stub with real ACS SDK:
...
# ---------------------------------------------------------------------------
# ENDPOINT 1 — IVR START
# WORKFLOW: Browser/VXML → POST /ivr/start → session + welcome prompt
# ---------------------------------------------------------------------------
@app.post("/ivr/start", summary="Start IVR — Welcome Prompt")
def start_ivr(p: StartRequest):
    """
    Entry point. Creates session, returns personalised welcome + main menu.
    M3 HOOK: acs_call_connection_id stored for ACS SDK use.
    """
    session = make_session(p.caller_id, p.language, p.acs_call_connection_id)
    log(session, "call_started", {"caller_id": p.caller_id})

    hour = (datetime.utcnow().hour + 5) % 24   # IST
    ...
    return ok(prompt, {
        ...
        "next_action": "collect_dtmf",    # M3: ACS calls start_recognizing_media()
    }, "welcome_menu")

# ---------------------------------------------------------------------------
# ENDPOINT 2 — UNIVERSAL DTMF HANDLER
# WORKFLOW: Caller presses key → POST /ivr/input → routes to service flow
# M3 HOOK: Replace digit matching with BAP NLU intent recognition
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ENDPOINT 3 — PNR STATUS
# M3 TODO: Replace mock with → requests.get("https://irctc-api/pnr/{pnr}")
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ENDPOINT 5 — TATKAL EMERGENCY BOOKING
# Creative: Urgency scoring + dynamic queue priority
# M3 TODO: Real-time Tatkal availability via IRCTC API
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ENDPOINT 6 — LIVE TRAIN TRACKING
# Creative: Delay prediction + next station ETA
# M3 TODO: Replace mock with NTES/IRCTC live tracking API
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ENDPOINT 7 — ACS/BAP BRIDGE STUB
# M3 TODO: Replace body with real ACS SDK:
#   from azure.communication.callautomation import CallAutomationClient, TextSource, RecognizeInputType
#   client = CallAutomationClient.from_connection_string(os.environ["ACS_CONN_STR"])
#   conn = client.get_call_connection(payload.acs_call_connection_id)
#   conn.start_recognizing_media(input_type=RecognizeInputType.DTMF,
#       play_prompt=TextSource(text=payload.tts_text, voice_name="en-IN-NeerjaNeural"),
#       max_tones_to_collect=payload.max_digits, operation_context="irctc_menu")
# ---------------------------------------------------------------------------
@app.post("/acs/bridge", summary="ACS/BAP Connector Bridge (M3 stub)")
...
# ---------------------------------------------------------------------------
# ENDPOINT 8 — SESSION INFO
# M3 AI layer reads/writes conversation slots through this
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# HEALTH CHECK — M4 deployment monitoring
# ---------------------------------------------------------------------------
...
"""
=============================================================================
IRCTC SMART IVR — INTEGRATION LAYER (MILESTONE 2) WORKFLOW
=============================================================================
...
Step 1 → Click "START CALL" button
         → Welcome prompt appears (Good Morning/Afternoon/Evening)
         → A unique Session ID is created

Step 2 → Press key "1" on the dialpad
         → IVR routes to PNR Status flow
         → Prompt asks for PNR number

Step 3 → Click "PNR STATUS" card (top left in IRCTC Services)
         → Enter PNR 4521679834
         → JSON response shows: Train, Coach, Berth, Platform details

Step 4 → Press key "2" on the dialpad
         → IVR routes to Smart Booking flow
         → Press 1 for Sleeper / 2 for 3AC / 3 for 2AC
         → Press quota (1 = General)
         → Press berth preference (1 = Lower)
         → Click "SMART BOOKING" card to confirm
         → JSON response shows: Booking ID, Fare, Payment link

Step 5 → Press key "3" on the dialpad
         → IVR routes to Tatkal Emergency Booking
         → Click "TATKAL EMERGENCY" card
         → JSON response shows: Ref ID, Surcharge, Queue Position, Urgency Score

Step 6 → Press key "4" on the dialpad
         → IVR routes to Live Train Tracking
         → Click "LIVE TRACKING" card
         → JSON response shows: Current Station, Next Station, Delay, ETA

Step 7 → Click "END CALL" to terminate session

NOTE: /acs/bridge endpoint is a STUB right now.
      In Milestone 3, real Azure ACS SDK will replace it.
=============================================================================
"""
